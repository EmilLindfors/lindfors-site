...
url: "https://lindfors.no/blog/reader-analytics/?issue=reader-analytics"
---
...
Sovereignty over your own data is the same question on the model side, where I [wrote about it](https://lindfors.no/blog/forking-codex-for-any-endpoint/?issue=reader-analytics) from the customer's chair. Here I am the vendor, and the readers are the customer, and the answer is the same: the data lives on a box I control, in a format I can query, and I can delete all of it with one command.
...
Third, it reads its configuration from `data-` attributes on its own script tag, written from the site config at build time. Not from an inline script, because the [content security policy](https://lindfors.no/blog/building-a-personal-blog-with-zola/?issue=reader-analytics) has no `unsafe-inline` and would drop a config object silently. The client token in there is a write-only ingest key, public by design.
...
What comes next is the newsletter. I said in [that post](https://lindfors.no/blog/self-hosted-newsletter/?issue=reader-analytics) that it has no open or click tracking and that I did not want any, and I have changed my mind on half of it. I do want to know whether an issue gets read, because that is the feedback the writing needs. I do not want to know it per reader. The plan is a parameter on the links in each issue, so a visit from the newsletter shows up in the view rows above as coming from that issue and nothing more. Per issue, not per person, is where the line goes, and the four words above are why it goes there.
...
*[Read the full post on the site](https://lindfors.no/blog/reader-analytics/?issue=reader-analytics) for math equations, citations, and interactive features.*
