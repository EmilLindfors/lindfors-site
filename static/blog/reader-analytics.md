...
canonical: https://lindfors.no/blog/reader-analytics/
---
...
Sovereignty over your own data is the same question on the model side, where I [wrote about it](https://lindfors.no/blog/forking-codex-for-any-endpoint/) from the customer's chair. Here I am the vendor, and the readers are the customer, and the answer is the same: the data lives on a box I control, in a format I can query, and I can delete all of it with one command.
...
Third, it reads its configuration from `data-` attributes on its own script tag, written from the site config at build time. Not from an inline script, because the [content security policy](https://lindfors.no/blog/building-a-personal-blog-with-zola/) has no `unsafe-inline` and would drop a config object silently. The client token in there is a write-only ingest key, public by design.
